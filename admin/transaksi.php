...
<hl class="m-0">Data Transaksi</hl>
</div>
</div><!-- /.row -->
</div><!-- /.container-fluid -->
</div>
...
while($d_tb_user_d = mysqli_fetch_array($tb_user)){ ?> <tr>
<td class="bg-dark text-center"><bx><?php echo $no++; ?></bx></td>
<td><?=$d_tb_transaksi['kode_invoice'] ?></td>
<td><?=$d_tb_member_d['nama'] ?></td><td>
<table>
<tr>
<td>
<?php
...
<select name="id_paket" class="form-control" id="id_outlet"
required>	<option>— Pilih Nama Paket —</option>
<?php
include "../koneksi.php";
...
<option value="selesai" <?php if('selesai' == $d_tb_transaksi['status']){ echo'selected' ; } ?>>Selesai</option>
	<option value="diambil"<?php if('diambil' ==
$d_tb_transaksi['status']){ echo 'selected' ; } ?>>Diambil</option>
	</select>
</div>
<div class="modal-footer">
...
<div class="modal fade" id="modal-batalkan<?php echo $d_tb_transaksi['id']; ?>">	<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
...
<b>
<?php
$tb_paket =mysqli_query($koneksi, "SELECT * FROM tb_paket where id='$d_tb_transaksi[id_paket]'");	while($d_tb_paket_d = mysqli_fetch_array($tb_paket)){ ?>
<?php
$a = $d_tb_paket_d['harga'];
}
$b = $d_tb_transaksi['qty'];
...
<div class="modal-content">
<div class="modal-header">
<h4 class="modal-title text-danger">Hapus Data Transaksi</h4>
...
<div class="modal-content">
<div class="modal-header">
<h4 class="modal-title text-warning">Edit Data Transaksi</h4>
<button type="button" class="close" data-dismiss="modal" aria-label="Close">	
<span aria-hidden="true">&times;</span>
</button>
</div>
...
while($d_tb_outlet_d_d =
mysqli_fetch_array($tb_outlet)){
?>
...
$d_tb_outlet['nama']?></option>	<?php } } ?>
</select>
</div>
<div class="form-group">
<label>Berat</label>
<input type="text" name="qty" class="form-control"
placeholder="Masukan Berat">	</div>
<div class="form-group">
<label>Biaya Tambahan</label>
