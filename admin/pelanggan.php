...
<hl class="m-0">Data Pelanggan</hl>
</div>
</div><!-- /.row -->
</div><!-- /.container-fluid -->
</div>
...
#modal-tambah"xi class="fas fa-plus"></i> Tambah Data</button>
</div>
</div>
<!-- /.card-header -->
<div class="card-body">
...
<h5xi class="icon fas fa-trash"></i> Sukses</h5>
Data berhasil di hapus
</div>
<?php } else if($_GET['info'] == "simpan"){ ?>
<div class="alert alert-success alert-dismissible">
<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
<h5xi class="icon fas fa-check"></i> Sukses</h5>
...
?>
<tr>
<td class="bg-dark text-center"> <b> <?php echo $no++; ?></bx></td>
<td><?=$d_tb_member['nama'] ?></td>
<td><?=$d_tb_member['alamat' ] ?></td>
<td>
<?php
if ($d_tb_member['jenis_kelamin'] == 'L') { ?> Laki-Laki
<?php } else { ?>
	Perempuan
<?php } ?>
</td>
...
<button class="btn btn-warning btn-sm text-white" data-toggle="modal" data-target="#modal-edit<?php echo $d_tb_member['id']; ?>"><i class="fas fa-edit" ></i> Ubah</button>
...
<div class="modal-content">
<div class="modal-header">
<h4 class="modal-title text-warning">Edit Data <?php echo$d_tb_member['nama'];?></h4>
