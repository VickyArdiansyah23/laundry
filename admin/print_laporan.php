...
<div class="alert alert-success alert-dismissible">
<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
<h5xi class="icon fas fa-edit"></i> Sukses</h5>
...
$tb_transaksi =mysqli_query($koneksi, "SELECT * FROM tb_transaksi where dibayar='dibayar' AND id_outlet='$id_outlet'");
...
id_outlet" required>	<option>— Pilih Nama Paket —</option>
<?php
include "•./koneksi.php";
...
<b>
<?php
$tb_paket =mysqli_query($koneksi, "SELECT * FROM tb_paket where id='$d_tb_transaksi[id_paket]'"); while($d_tb_paket_d = mysqli_fetch_array($tb_paket)){ ?> <?php
$a = $d_tb_paket_d['harga'];
}
$b = $d_tb_transaksi['qty'];
$c = $d_tb_transaksi['biaya_tambahan'];
$total = ($a*$b)+$c;
echo "Rp. $total"; ?> </b> dari <b><?=$d_tb_member_d['nama'] ?></b>...?</p>
...
<div class="modal-footer justify-content-between">
<button type="button" class="btn btn-default" data-dismiss="modal">Keluar</button>
<a href="hapus transaksi.php?id=<?php echo $d_tb_transaksi['id']; ?>" class="btn btn-primary">Hapus</a>
	</div>
</div>
</div>
</div>
...
<input type="text" name="kode invoice" class="form-control" value="<?php echo $d_tb_transaksi['kode_invoice']; ?>" readonly= >
</div>
	<div class="form-group">
...
if($d_tb_member['id'] == $d_tb_transaksi['idjnember']){ echo 'selected'; } ?>><?= $d_tb_member['nama']?></option>
<?php } ?>
</select>
</div>
<div class="form-group">
...
<input type="text" name="biaya tambahan" value="<?php
...
tbjnember");	while($d_tb_member = mysqli_fetch_array($tb_member)){ ?>
...
?>
<option value="<?=$d_tb_outlet['id']?>"><?=$d_tb_outlet['nama']?></option>	<?php } } ?>
</select>
</div>
<div class="form-group">
<label>Berat</label>
...
dismiss="modal">Keluar</button>	<button type="submit" class="btn btn-primary">Simpan</button>	
</div>
</form>
</div>
</div>
