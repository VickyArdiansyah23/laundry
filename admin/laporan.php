...
<hl class="m-0">Laporan Transaksi</hl>
</div>
</div><!-- /.row -->
</div><!-- /.container-fluid -->
</div>
...
<a href="print_laporan.php" class="btn btn-primary btn-sm" target="blank_" class="fas fa-print"></i> Print Data</a>
</div>
</div>
<!-- /.card-header -->
<div class="card-body">
...
while ($d_tb_transaksi = mysqli_fetch_array($tb_transaksi)) {
    ...
    <tr>
    <td class="bg-dark text-center"> <bx?php echo $no++; ?></b></td>	
  ...
  while($d_tb_paket_d = mysqli_fetch_array($tb_paket)){ ?>
    ...
    if ($d_tb_transaksi['id_paket'] == '0') { ?>
 	<?php } else { ?>
    <?php
    $tb_paket	=mysqli_query($koneksi, "SELECT * FROM tb_paket where
    id='$d_tb_transaksi[id_paket]'");
    while($d_tb_paket_d = mysqli_fetch_array($tb_paket)){ $a = $d_tb_paket_d['harga'];
    }
...
<p>Apakah	anda yakin akan menghapus data <b><?php echo $d_tb_transaksi['kode_invoice']; ?></b> ini...?</p>
    ...
    <div class="modal-footer justify-content-between">
<button type="button" class="btn btn-default" data-dismiss="modal">Keluar</button>
<a href="hapus_transaksi.php?id=<?php echo $d_tb_transaksi['id']; ?>"class="btn btn-pnmary">Hapus</a>
</div>
</div>
</div>
</div>
